...
def analyze_extraction(json_path: Path):
    ...
    print(
        f"Avg chars/block: {total_chars/total_blocks:.1f}" if total_blocks > 0 else ""
    )
    ...
    for page in data.get("pages", []):
        ...
        for block in page.get("blocks", []):
            ...
            if any(
                ord(c) > 127 and ord(c) not in [176, 178, 179, 8451, 8457] for c in text
            ):
                suspicious_chars.append((page_num, text[:50]))

    # Report findings
    print(
        f"Empty blocks: {empty_blocks} ({100*empty_blocks/total_blocks:.1f}%)"
        if total_blocks > 0
        else ""
    )
    ...
    if font_sizes:
        ...
        print(
            f"  Top sizes: {', '.join(f'{s:.1f}' for s, _ in size_counter.most_common(5))}"
        )
    ...
    unit_stats = unit_normalizer.get_unit_statistics(
        all_text[:100000]
    )  # Sample first 100k chars
...
def main():
    ...
    print(f"Total blocks: {total_blocks:,}")
    print(
        f"Empty blocks: {total_empty} ({100*total_empty/total_blocks:.1f}%)"
        if total_blocks > 0
        else ""
    )
