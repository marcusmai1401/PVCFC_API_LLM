#!/usr/bin/env pwsh
# Run BGE reranking evaluation (chunk-level)

Write-Host "=" * 80 -ForegroundColor Cyan
Write-Host "BGE RERANK EVALUATION - CHUNK-LEVEL RERANKING" -ForegroundColor Cyan
Write-Host "=" * 80 -ForegroundColor Cyan
Write-Host ""

# Set BGE rerank config (chunk-level)
...
# Run BGE rerank queries
$inputFile = "artifacts/qa/filtered_qa_set.jsonl"
$outputFile = "artifacts/eval/run_bge_chunk.jsonl"
$limit = 30

Write-Host "Running batch queries with BGE reranking..." -ForegroundColor Yellow
Write-Host "  Input: $inputFile" -ForegroundColor White
Write-Host "  Output: $outputFile" -ForegroundColor White
Write-Host "  Limit: $limit queries" -ForegroundColor White
Write-Host ""
Write-Host "⚠️  First query will be slower (model loading)" -ForegroundColor Yellow
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "✓ BGE CHUNK-LEVEL RERANK RUN COMPLETE" -ForegroundColor Green
    Write-Host "=" * 80 -ForegroundColor Green
    Write-Host ""
    Write-Host "Results saved to: $outputFile" -ForegroundColor White
    Write-Host ""
    Write-Host "Next step: Compare results with:" -ForegroundColor Yellow
    Write-Host "  .\scripts\eval_bge_rerank\04_compare_results.ps1" -ForegroundColor Cyan
} else {
    Write-Host ""
    Write-Host "=" * 80 -ForegroundColor Red
    Write-Host "✗ BGE RERANK RUN FAILED" -ForegroundColor Red
    Write-Host "=" * 80 -ForegroundColor Red
    exit 1
}
