"""Index test chunks into OpenSearch and Weaviate
Temporary script to test indexing with ingestion_test output
"""
import json
import sys
...
CHUNKS_FILE = PROJECT_ROOT / "artifacts" / "ingestion_test" / "chunks" / "chunks.jsonl"
BATCH_SIZE = 100
...
def index_chunks():
    """Main indexing logic"""
    logger.info("=" * 80)
    logger.info("INDEXING TEST CHUNKS")
    logger.info("=" * 80)
    ...
    with open(CHUNKS_FILE, "r", encoding="utf-8") as f:
        for line_num, line in enumerate(tqdm(f, desc="Indexing chunks"), 1):
            try:
                ...
                # Remove None values
                weaviate_properties = {
                    k: v for k, v in weaviate_properties.items() if v is not None
                }

                # Generate UUID for Weaviate
                uuid = generate_uuid5(weaviate_properties)
