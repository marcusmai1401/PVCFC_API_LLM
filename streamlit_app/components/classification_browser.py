...
class DocumentExplorer:
    ...
    def fetch_taxonomy(self) -> Optional[Dict]:
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/classification/taxonomy", timeout=10
            )
    ...
    def trigger_classification(self, doc_id: str, pdf_path: str) -> Optional[Dict]:
        ...
        try:
            response = requests.post(
                ...
                json={"doc_id": doc_id, "pdf_path": pdf_path, "force_reclassify": True},
                timeout=60,
            )
            if response.status_code == 200:
                return response.json()
    ...
    def get_mock_documents(self) -> Dict[str, Dict[str, List[Dict]]]:
        ...
        # Demo data structure with dynamic paths
        return {
            "ENGINEERING_DESIGN": {
                "P&ID": [
                    {
                ...
                "Drawing": [
                    {
                        ...
                        "pdf_path": f"{base_path}/K06101/Drawings/K06101_GA_Drawing.pdf",
                    },
                ],
                "Technical Data": [],
            },
                ...
                "Vendor Manual": [
                    {
                        ...
                        "pdf_path": f"{base_path}/K06101/Vendor/Hitachi_Operation_Manual.pdf",
                    },
                ],
            },
                ...
                "Unknown": [
                    {
                        ...
                        "pdf_path": f"{base_path}/K06101/Misc/Misc_Document.pdf",
                    },
                ],
            },
        }
    ...
    def render_status_badge(self, status: str) -> str:
        ...
        """
        icon, color, label = STATUS_BADGES.get(status, ("❓", "#8e8e93", "Unknown"))
    ...
    def render_document_row(self, doc: Dict) -> None:
        ...
        with col2:
            if st.button(
                "🔄 Re-classify", key=f"reclassify_{row_key}", use_container_width=True
            ):
    ...
    def render_document_preview(self) -> None:
        ...
        with st.container():
            ...
            with col_close:
                if st.button(
                    "✕ Close Preview", key="close_preview", use_container_width=True
                ):
            ...
            with col2:
                ...
                confidence = doc.get("classification_confidence", 0.0)
                st.markdown(
                    self.render_confidence_bar(confidence), unsafe_allow_html=True
                )
            ...
            with col_reclassify:
                if st.button(
                    "🔄 Re-classify", key="preview_reclassify", use_container_width=True
                ):
                    with st.spinner("Running classification pipeline..."):
                        result = self.trigger_classification(
                            doc.get("doc_id", ""), doc.get("pdf_path", "")
                        )
                        if result:
                            st.success(
                                f"✅ Classified as: {result.get('category')} / {result.get('doc_type')} "
    ...
    def render_tree_view(
        self, documents_by_category: Dict[str, Dict[str, List[Dict]]]
    ) -> None:
        """
    ...
    def render_filter_sidebar(self) -> Dict:
        ...
        selected_category = st.sidebar.selectbox(
            "Category", categories, key="filter_category"
        )

        # Doc type filter (depends on category)
        if selected_category and selected_category != "All":
            doc_types = ["All"] + self.taxonomy.get_doc_types_for_category(
                selected_category
            )
        else:
        ...
        selected_doc_type = st.sidebar.selectbox(
            "Document Type", doc_types, key="filter_doc_type"
        )
    ...
    def render(self) -> None:
        ...
        if api_data:
            ...
            if filters.get("category"):
                documents_by_category = {
                    k: v
                    for k, v in documents_by_category.items()
                ...
                for category in documents_by_category:
                    for doc_type in documents_by_category[category]:
                        documents_by_category[category][doc_type] = [
                            doc
                            for doc in documents_by_category[category][doc_type]
