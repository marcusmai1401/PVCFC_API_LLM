"""
Document Classifier using Gemini 3 Flash Preview
...
class DocumentClassifierProtocol(Protocol):
    ...
    def classify(
        self, page_images: List[bytes], filename: str, metadata: Optional[Dict] = None
    ) -> ClassificationResult:
...
class DocumentClassifier:
    """
    AI-powered document classifier using Gemini 3 Flash Preview
...
Analyze the document pages now:"""
    ...
    def _call_gemini(self, prompt: str, images: List[bytes]) -> str:
    ...
    def _parse_response(self, response: str, filename: str) -> ClassificationResult:
        ...
        try:
            ...
            if not self.taxonomy.is_valid_category(category):
                logger.warning(
                    f"Invalid category '{category}' from Gemini, using UNCATEGORIZED"
                )
                category = DocumentCategory.UNCATEGORIZED.value
    ...
    def _apply_dominant_content_rule(self, page_analysis: List[PageAnalysis]) -> str:
        """
