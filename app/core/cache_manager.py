...
class RetrievalCache:
    ...
    def _make_key(
        ...
        """
        Generate cache key from query + query_type + filters + k

        Args:
            query: Query string (normalized)
            query_type: Query type (pid, technical_doc, etc.) for cache separation
        ...
        key_dict = {
            ...
            "query_type": query_type or "default",  # Include query_type in cache key
            "filters": filters or {},
            "k": k,
        }
    ...
    def get(
        ...
        Args:
            query: Query string
            query_type: Query type for cache separation
        ...
        """
        key = self._make_key(query, query_type, filters, k)
