...
class VectorExtractor:
    ...
    def __init__(self, merge_threshold: float = 10.0, fix_hyphenation: bool = True):
        """
    ...
    def extract_from_pdf(self, pdf_path: str | Path) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {"file_path": str(pdf_path), "total_pages": len(doc), "pages": []}
            ...
            results["statistics"] = {
                ...
                "avg_blocks_per_page": total_blocks / total_pages
                if total_pages > 0
                else 0,
            }
    ...
    def _merge_nearby_blocks(self, blocks: List[TextBlock]) -> List[TextBlock]:
        ...
        for next_block in blocks[1:]:
            ...
            if (
                y_distance < self.merge_threshold
                and x_distance < self.merge_threshold * 2
            ):
                # Merge blocks
    ...
    def extract_with_structure(self, pdf_path: str | Path) -> Dict[str, Any]:
        ...
        for page in results["pages"]:
            all_blocks.extend(
                [
                    TextBlock(
                        **{k: v for k, v in b.items() if k != "bbox"},
                        bbox=tuple(b["bbox"]),
                    )
