...
class UnitNormalizer:
    ...
    def _apply_unit_mappings(self, text: str) -> str:
        ...
        sorted_mappings = sorted(
            self.UNIT_MAPPINGS.items(), key=lambda x: len(x[0]), reverse=True
        )
        ...
        for old_unit, new_unit in sorted_mappings:
            ...
            text = re.sub(
                ...
                new_unit,
                text,
                flags=re.IGNORECASE if self.normalize_case else 0,
            )
    ...
    def _fix_number_unit_spacing(self, text: str) -> str:
        ...
        text = re.sub(
            r"(\d)\s+([°℃℉])", r"\1\2", text
        )  # No space before degree symbols
    ...
    def get_unit_statistics(self, text: str) -> Dict[str, any]:
        ...
        return {
            ...
            "most_common": max(unit_groups.items(), key=lambda x: len(x[1]))[0]
            if unit_groups
            else None,
