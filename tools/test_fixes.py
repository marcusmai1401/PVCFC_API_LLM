...
def test_hyphenation_fix():
    ...
    try:
        ...
        print(
            f"[OK] Hyphenation fix works - extracted {result['statistics']['total_blocks']} blocks"
        )
        ...
        for page in result["pages"]:
            if "-" in page["full_text"]:
                print(
                    f"  Page {page['page_num']}: Found hyphens in text (OK if intentional)"
                )
    ...
    try:
        ...
        for page in result["pages"]:
            ...
            if len(blocks) > 1:
                ...
                if out_of_order == 0:
                    print(
                        f"[OK] Page {page['page_num']}: All blocks in correct reading order"
                    )
                else:
                    print(
                        f"[WARN] Page {page['page_num']}: {out_of_order} blocks may be out of order"
                    )
    ...
    try:
        ...
        for page in result["pages"]:
            ...
            if len(block_nums) == len(set(block_nums)):
                ...
                print(
                    f"[FAIL] Page {page['page_num']}: Duplicate block_num values found"
                )
                return False
