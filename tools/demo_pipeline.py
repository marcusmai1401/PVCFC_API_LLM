...
def process_document(pdf_path: Path):
    ...
    print(
        f"   Extracted: {stats.get('total_blocks', 0):,} blocks, {stats.get('total_characters', 0):,} characters"
    )
    ...
    chunker = HierarchicalChunker(
        max_chunk_size=500, chunk_overlap=50, use_token_count=True  # tokens
    )
    ...
    print(
        f"   Size range: {chunk_stats['min_chunk_size']}-{chunk_stats['max_chunk_size']} tokens"
    )
...
def main():
    ...
    queries = ["steam turbine", "CO2 compressor", "temperature", "pressure", "KT06101"]
