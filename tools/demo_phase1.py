...
def demo_text_extraction():
    ...
    for structure_type, blocks in structure_groups.items():
        ...
        for block in blocks[:3]:  # Show first 3 of each type
            ...
            console.print(
                f"    [dim]BBox: {block['bbox']}, Font: {block.get('font_size', 'N/A')}pt[/dim]"
            )
...
def demo_mixed_pdf():
    ...
    for page_data in extraction["pages"]:
        ...
        if text_length > 0:
            preview = (
                page_data["full_text"][:150] + "..."
                if len(page_data["full_text"]) > 150
                else page_data["full_text"]
            )
