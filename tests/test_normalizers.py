...
from app.rag.normalizers.tag_normalizer import (
    ISA5_1_TagNormalizer,
    TagNormalizer,
    TagPattern,
)
from app.rag.normalizers.text_normalizer import TechnicalTextNormalizer, TextNormalizer
...
class TestISA5_1_TagNormalizer:
    ...
    def test_isa_patterns(self):
        ...
        assert "flow" in grouped or any(
            "F" in tag for tag in [t["normalized"] for t in tags]
        )
        assert "pressure" in grouped or any(
            "P" in tag for tag in [t["normalized"] for t in tags]
        )
