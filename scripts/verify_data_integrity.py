    ...
    try:
        ...
        if client.indices.exists(index=opensearch_index):
            ...
            if count != total_chunks:
                logger.warning(f"⚠️  OpenSearch mismatch! Expected {total_chunks}, got {count}")
            else:
    ...
    if bm25_path.exists():
        with open(bm25_path, 'r', encoding='utf-8') as f:
            ...
            if len(docs) != total_chunks:
                logger.warning(f"⚠️  BM25 mismatch! Expected {total_chunks}, got {len(docs)}")
            else:
    ...
    if faiss_path.exists():
        with open(faiss_path, 'r', encoding='utf-8') as f:
            ...
            if len(texts) != total_chunks:
                logger.warning(f"⚠️  FAISS mismatch! Expected {total_chunks}, got {len(texts)}")
            else:
