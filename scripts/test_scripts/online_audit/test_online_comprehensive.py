...
logger.add(
    PROJECT_ROOT / "logs" / f"online_audit_{int(time.time())}.log",
    level="DEBUG"
)
...
class ComprehensiveOnlineAuditor:
    ...
    def check_preconditions(self) -> bool:
        ...
        try:
            ...
            findings.append({"check": "server_running", "status": "FAIL", "error": "Connection refused"})
            all_ok = False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                findings.append({"check": "index_loaded", "status": "PASS", "data": data})
            else:
            ...
            findings.append({"check": "index_loaded", "status": "WARNING", "error": str(e)})
        
        try:
            # Check 3: doc_id_map.json
            logger.info("  Checking doc_id_map...")
            doc_id_map_path = PROJECT_ROOT / "artifacts" / "ingestion" / "doc_id_map.json"
            if doc_id_map_path.exists():
                ...
                findings.append({"check": "doc_id_map", "status": "PASS", "entries": len(doc_id_map)})
            else:
                logger.warning(f"  ⚠ doc_id_map.json not found")
                logger.warning(f"    Citations will not have pdf_path")
                findings.append({"check": "doc_id_map", "status": "WARNING", "note": "pdf_path unavailable"})
                
        except Exception as e:
            logger.warning(f"  ⚠ Could not load doc_id_map: {e}")
            findings.append({"check": "doc_id_map", "status": "WARNING", "error": str(e)})
    ...
    def audit_basic_queries(self):
        ...
        for test in test_queries:
            try:
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.info(f"  ✓ {test['label']}: {latency:.0f}ms, {len(data.get('citations', []))} citations")
                    
                    self.metrics["latencies"].append(latency)
                    self.metrics["citation_counts"].append(len(data.get("citations", [])))
                ...
                findings.append({"query_type": test["label"], "status": "FAIL", "error": str(e)})
        
        self.test_results["basic_queries"] = {
            "status": "PASS" if all(f.get("status") == "PASS" for f in findings) else "FAIL",
            "findings": findings
        }
    ...
    def audit_error_handling(self):
        ...
        try:
            ...
            if response.status_code in [400, 422]:
                ...
                logger.warning(f"  ⚠ Unexpected status for empty query: {response.status_code}")
        ...
        self.test_results["error_handling"] = {
            "status": "PASS" if findings and findings[0].get("status") == "PASS" else "FAIL",
            "findings": findings
        }
    ...
    def audit_retrieval_rerank(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                logger.info(f"  ✓ EN retrieval+rerank: {len(data.get('citations', []))} citations")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(citations) > 0 and len(answer) > 10:
                    logger.info(f"  ✓ VI retrieval+rerank: {len(citations)} citations, no NaN")
                    findings.append({"test": "vi_rerank_no_nan", "status": "PASS"})
                else:
                    logger.error(f"  ✗ VI returned empty results (possible NaN issue)")
                    findings.append({"test": "vi_rerank_no_nan", "status": "FAIL", "issue": "Empty results"})
            else:
            ...
            if response.status_code == 200:
                ...
                if "vision_generation" in meta:
                    ...
                    if enabled and len(pages_used) > 0:
                        ...
                        findings.append({"test": "vision_enabled", "status": "PASS", "pages": len(pages_used)})
                    elif not enabled:
                        logger.info(f"  ℹ Vision not triggered (may lack pdf_path)")
                        findings.append({"test": "vision_enabled", "status": "INFO", "note": "Not triggered"})
                    else:
                        findings.append({"test": "vision_enabled", "status": "INFO"})
                else:
                    logger.warning(f"  ⚠ vision_generation not in meta")
                    findings.append({"test": "vision_info_present", "status": "WARNING"})
            ...
            if response.status_code == 200:
                ...
                if not missing:
                    ...
                    if "breakdown" in meta:
                        ...
                        logger.info(f"    - transform: {breakdown.get('transform_ms', 0):.0f}ms")
                        logger.info(f"    - retrieve: {breakdown.get('retrieve_ms', 0):.0f}ms")
                        logger.info(f"    - rerank: {breakdown.get('rerank_ms', 0):.0f}ms")
                        logger.info(f"    - generate: {breakdown.get('generate_ms', 0):.0f}ms")
                        
                        # Verify breakdown sum ≈ total latency
                        breakdown_sum = sum(breakdown.values())
                        total_latency = meta.get("latency_ms", 0)
                        if abs(breakdown_sum - total_latency) / total_latency < 0.2:  # Within 20%
                            logger.info(f"  ✓ Breakdown sum matches total ({breakdown_sum:.0f} ≈ {total_latency:.0f})")
                            findings.append({"check": "breakdown_accuracy", "status": "PASS"})
                        else:
                            logger.warning(f"  ⚠ Breakdown sum mismatch: {breakdown_sum:.0f} vs {total_latency:.0f}")
                            findings.append({"check": "breakdown_accuracy", "status": "WARNING"})
                    else:
                        logger.warning(f"  ⚠ Latency breakdown not present")
                        findings.append({"check": "latency_breakdown", "status": "WARNING"})
                    ...
                    if "trace_id" in meta:
                        ...
                        findings.append({"check": "trace_id_present", "status": "WARNING"})
                        
                else:
                    logger.error(f"  ✗ Missing fields: {missing}")
                    findings.append({"check": "schema_complete", "status": "FAIL", "missing": missing})
    ...
    def audit_metrics_trace(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f"  ✓ /metrics endpoint available ({len(metrics_text)} chars)")
                ...
                if "# HELP" in metrics_text or "# TYPE" in metrics_text:
                    ...
                    findings.append({"check": "metrics_prometheus", "status": "WARNING"})
            else:
        ...
        self.test_results["metrics_trace"] = {
            "status": "PASS",
            "findings": findings
        }
    ...
    def _generate_report(self) -> Dict:
        ...
        # Save report
        report_path = PROJECT_ROOT / "reports" / "test_results" / f"online_comprehensive_audit_{int(time.time())}.json"
        report_data = {
