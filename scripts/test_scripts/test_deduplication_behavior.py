    ...
    if result.returncode == 0:
        ...
        if doc_id_map_file.exists():
            ...
            if processed_count == 1:
                logger.info("  ✅ PASS: Exact duplicate was skipped (file_hash dedup works)")
            elif processed_count == 2:
                logger.warning("  ⚠️  ISSUE: Both files processed (file_hash dedup NOT working)")
...
def main():
    ...
    if processed_count == 1:
        ...
        logger.error(f"❌ UNEXPECTED: {processed_count} files processed (expected 1 or 2)")
        return 1
