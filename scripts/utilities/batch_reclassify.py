...
Note:
    Default PDF directory is D:/Data_Raw (PVCFC production data location)
"""
...
def get_all_doc_ids(os_client: OpenSearch, index_name: str = "rag_chunks") -> List[Dict[str, Any]]:
    """
    ...
    if sample['hits']['hits']:
        ...
        has_metadata_doc_id = 'metadata' in sample_doc and 'doc_id' in sample_doc.get('metadata', {})
        logger.info(f"Schema detection: root_doc_id={has_root_doc_id}, metadata_doc_id={has_metadata_doc_id}")
    else:
    ...
    if has_root_doc_id:
        ...
        doc_id_field = "doc_id"
        source_fields = ["doc_id", "file_name", "category", "doc_type", "classification_status"]
    else:
        # Fields under metadata (new schema)
        doc_id_field = "metadata.doc_id"
        source_fields = [
            "metadata.doc_id", "metadata.filename", "metadata.category",
            "metadata.doc_type", "metadata.classification_status", "metadata.file_path"
        ]
            ...
            try:
                collection.data.update(
                    ...
                    properties={
                        "category": classification.get("category", "UNCATEGORIZED"),
                        "doc_type": classification.get("doc_type", "Unknown"),
                        "classification_status": classification.get("status", "classified"),
                    }
                )
                updated_count += 1
            except Exception as e:
    ...
    for idx, doc_info in enumerate(documents_to_process):
        ...
        try:
            ...
            if not dry_run and (idx + 1) % batch_size == 0:
                ...
                logger.info(f"  💾 Checkpoint saved ({len(processed_doc_ids)} processed)")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="D:/Data_Raw",
        help="Directory containing PDF files (default: D:/Data_Raw)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
