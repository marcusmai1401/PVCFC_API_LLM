...
def clean_weaviate():
    ...
    try:
        ...
        logger.info(f"Collection '{settings.weaviate_collection}': {object_count} objects")
        ...
        if object_count != 0:
            ...
            try:
                ...
                logger.success(f"✅ Deleted {deleted_count} objects from '{settings.weaviate_collection}'")
            except Exception as e:
                # Method 2: Fallback - delete collection and recreate
                logger.warning(f"Batch delete failed: {e}")
                logger.info("Trying alternative method: delete and recreate collection...")
                ...
                try:
                    ...
                    logger.success(f"✅ Recreated empty collection '{settings.weaviate_collection}'")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("")
    logger.warning("⚠️  This will delete ALL indexed data from OpenSearch and Weaviate!")
    ...
    if opensearch_success and weaviate_success:
        ...
        logger.success("You can now run: python scripts/utilities/index_production_chunks.py")
        logger.success("")
    else:
        logger.error("")
